...
stage('Build & Test') {
	def buildTypes = ['asan', 'ccmode', 'dev', 'production', 'hwhsm', 'pkcs11']
	def testModes = [asan: 'dev', pkcs11: 'dev']
	// Extra VM-container-tests.sh flags per buildtype. The pkcs11 image runs the
	// normal dev test suite against a softhsm2-backed PKCS#11 container token.
	def extraOpts = [pkcs11: '--test-pkcs11 libsofthsm2.so']
	def hsmEnvs = [
